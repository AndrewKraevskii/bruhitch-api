...
const subscribePrediction = async (id: string, ws: WebSocket) => {
  //#region Get WsClient
  ...
  const begin = await fetch('https://api.twitch.tv/helix/eventsub/subscriptions', {
    ...
    body: JSON.stringify({
      type: EventSubType.PredictionBegin,
      version: '1',
      condition: {
  ...
  if (begin.status !== 202) {
    return wsClient.ws.send(
      getErrorMessage(begin.data?.error ?? 'Error on subscribe on channel.prediction.begin')
    );
  }
  //#endregion
  ...
  const progress = await fetch('https://api.twitch.tv/helix/eventsub/subscriptions', {
    ...
    body: JSON.stringify({
      type: EventSubType.PredictionProgress,
      version: '1',
      condition: {
        broadcaster_user_id: wsClient.channelId
      },
  ...
  if (progress.status !== 202) {
    return wsClient.ws.send(
      getErrorMessage(progress.data?.error ?? 'Error on subscribe on channel.prediction.progress')
    );
  }
  //#endregion
  ...
  const end = await fetch('https://api.twitch.tv/helix/eventsub/subscriptions', {
    ...
    body: JSON.stringify({
      type: EventSubType.PredictionEnd,
      version: '1',
      condition: {
        broadcaster_user_id: wsClient.channelId
      },
  ...
  if (end.status !== 202) {
    return wsClient.ws.send(
      getErrorMessage(end.data?.error ?? 'Error on subscribe on channel.prediction.end')
    );
  }
  //#endregion
...
export default subscribePrediction;
